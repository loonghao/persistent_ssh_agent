...
def test_get_available_keys(ssh_manager, tmp_path):
    ...
    key_pairs = {"id_ed25519": "ED25519 KEY", "id_rsa": "RSA KEY", "id_ecdsa": "ECDSA KEY", "id_dsa": "DSA KEY"}
    ...
    with patch.object(ssh_manager, "_ssh_dir", ssh_dir), patch.object(ssh_manager.ssh_key_manager, "ssh_dir", ssh_dir):
        ...
        assert (
            key_positions[expected_ed25519_path] < key_positions[expected_rsa_path]
        )  # Ed25519 should be preferred over RSA
...
def test_get_available_keys_error_handling(ssh_manager):
    ...
    with patch.object(ssh_manager, "_ssh_dir", nonexistent_path), patch.object(
        ssh_manager.ssh_key_manager, "ssh_dir", nonexistent_path
    ):
        available_keys = ssh_manager._get_available_keys()
...
def test_get_identity_file_with_multiple_keys(ssh_manager, tmp_path):
    ...
    key_pairs = {"id_ed25519": "ED25519 KEY", "id_rsa": "RSA KEY"}
...
def test_get_identity_file_config_priority(ssh_manager, tmp_path):
    ...
    with patch.dict(os.environ, {"SSH_IDENTITY_FILE": str(key_file)}), patch.object(ssh_manager, "_ssh_dir", ssh_dir):
        identity_file = ssh_manager._get_identity_file("example.com")
...
def test_security_key_detection(ssh_manager, tmp_path):
    ...
    security_keys = {"id_ecdsa_sk": "ECDSA-SK KEY", "id_ed25519_sk": "ED25519-SK KEY"}
