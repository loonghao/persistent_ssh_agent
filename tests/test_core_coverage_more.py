...
def test_load_agent_info_expired():
    ...
    mock_file.read.return_value = '{"SSH_AUTH_SOCK": "/tmp/ssh-agent.sock", "SSH_AGENT_PID": "123", "timestamp": 0, "platform": "posix"}'
...
def test_load_agent_info_platform_mismatch():
    ...
    mock_file = MagicMock()
    mock_file.__enter__.return_value = mock_file
    mock_file.read.return_value = '{"SSH_AUTH_SOCK": "/tmp/ssh-agent.sock", "SSH_AGENT_PID": "123", "timestamp": 9999999999, "platform": "posix"}'
...
def test_load_agent_info_ssh_add_failure():
    ...
    mock_file = MagicMock()
    mock_file.__enter__.return_value = mock_file
    mock_file.read.return_value = '{"SSH_AUTH_SOCK": "/tmp/ssh-agent.sock", "SSH_AGENT_PID": "123", "timestamp": 9999999999, "platform": "nt"}'
...
def test_start_ssh_agent_reuse_without_key():
    ...
    with patch.object(agent, "_load_agent_info", return_value=True):
        ...
        with patch.object(agent, "_verify_loaded_key", return_value=False):
            ...
            mock_result.stdout = "SSH_AUTH_SOCK=/tmp/ssh-agent.sock; export SSH_AUTH_SOCK;\nSSH_AGENT_PID=123; export SSH_AGENT_PID;"
            with patch.object(agent, "run_command", return_value=mock_result):
